...
export function installToolErrorCapture(ctx, cfg, log, classifier, policy) {
    ...
    const recordFailure = async (exec, errorMessage, info) => {
        ...
        const agentId = e.agent
            ? typeof e.agent === 'string'
                ...
                : (e.agent.id ?? null)
            : null;
        const c = { toolName, info, message: errorMessage, sessionId, agentId };
        ...
        if (p.record) {
            ...
            total++;
            if (p.log) {
                await toolErrLog.write('WARN', `[${klass}] ${toolName} session=${sessionId ?? '-'} agent=${agentId ?? '-'} ${errorMessage}`);
            }
    ...
    const offExecute = ctx.on?.('tools/execute', async (exec, next) => {
        ...
        try {
            ...
            const info = err && typeof err === 'object' && (err.name !== undefined || err.code !== undefined)
            ...
            try {
                ...
                void log.warn(`tool error capture (throw path) failed: ${recErr.message}`);
            }
            threw = true;
            throw e;
        }
