...
import { StatePaths, doctorBase, ensureDir, readFileOrNull, tailFile, pidAlive, writeFileAtomic, logPath, readLastTickAt, lastTickPath, readStartedAt, startedAtPath, writeWebPid, } from './state.js';
 ...
 */
export function looksLikeDshWeb(argv = process.argv) {
    ...
    if (first !== 'web')
        return false;
    // The first non-flag subcommand is "web", but is that token actually a
    // dsh subcommand, or just the argument of some other script
    // (`node script.js web`)? Require argv[1] to be the dsh executable
    // itself (base name dsh / dsh.js / dsh.cmd / dsh.exe). dsh may be
    // launched through a relative/absolute path or via a versioned bin
    // prefix, so only the base name is inspected.
    const prog = String(argv[1] ?? '');
    const base = (prog.match(/[^\\/]*$/) ?? [''])[0].toLowerCase();
    if (base === 'dsh' || base === 'dsh.js' || base === 'dsh.cmd' || base === 'dsh.exe')
        return true;
    ...
    if (base === 'bin.js' && /node_modules[\\/]+@deepseek-ai[\\/]+dsh[\\/]/.test(prog))
        return true;
    return false;
}
// ---------------------------------------------------------------------------
...
export function apply(ctx, config) {
    ...
    // the standalone daemon's EADDRINUSE kill-branch has a real pid to
    // signal. But NOT every process that mounts the web profile is dsh
    // web: `dsh plugin add`, an `npx` install, or a one-shot `node` that
    // evaluates the profile all run apply() too, with a short-lived pid
    // that dies before dsh web does — that clobbers the marker (observed
    // live: a CLI runner's pid overwrote the running web's, leaving a
    // stale dead pid behind while the real web kept serving). So only a
    // process whose argv looks like a genuine `dsh web` launch writes.
    if (looksLikeDshWeb()) {
        void writeWebPid(process.pid).catch((e) => {
            void log.warn(`writeWebPid failed: ${e.message}`);
        });
    }
    ...
    if (cfg.autoInstall) {
        void maybeAutoInstall(log, cfg).catch((e) => {
            void log.warn(`auto-install check failed: ${e.message}`);
        });
    }
    else {
    ...
    harness.registerTool(ctx, defineTool({
        ...
        parameters: {
            logLines: { type: 'number', description: 'How many log lines to consider. Defaults to Config.triageLogLines.', default: 0 },
        },
        output: JSON_OUTPUT,
        execute: execBody(async (args, _exec) => {
            const resolved = resolveConfig(cfg);
            const requested = Number(args.logLines ?? 0);
            // 0 means "use configured default"; otherwise honour caller (clamped).
            const n = requested > 0
                ? Math.max(10, Math.min(50_000, requested))
                : Math.max(10, Math.min(50_000, resolved.triageLogLines));
    ...
    harness.registerTool(ctx, defineTool({
        ...
        description: 'Return the last N lines of one of the doctor-managed log files. ' +
        ...
        execute: execBody(async (args, _exec) => {
            ...
            const allowed = ['web', 'watchdog', 'doctor', 'tool-errors'];
            if (!allowed.includes(raw)) {
                return { ok: false, reason: `unknown kind "${raw}"`, lines: [] };
            }
            const kind = raw;
...
function formatUptime(sec) {
    ...
    let start = 0;
    while (start < units.length && units[start][0] === 0)
        start++;
    if (start >= units.length)
        return '0s';
    let end = Math.min(units.length, start + 3);
    while (end > start && units[end - 1][0] === 0)
        end--;
