...
async function platformServiceInstalled() {
 ...
 */
export async function maybeAutoInstall(log, cfg = ConfigDefaults) {
    ...
    await log.info(`auto-install triggered (script=${scriptExists}, ` +
...
function spawnHelper(log, _cfg) {
    try {
        ...
        const selfEntry = require.resolve('./auto-install-helper.js');
        ...
        void log.warn(`auto-install failed to spawn helper: ${e.message}`);
        return false;
    }
}
