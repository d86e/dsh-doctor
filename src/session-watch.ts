  ...
  const offEvent = (ctx as any).on?.('session/event', (session: DshSession, event: DshSessionEvent) => {
    ...
    switch (ev.type) {
      case 'turn/start':
        ...
        // A new turn has started, so any failure recorded against the
        // previous turn is now a historical fact, not an active stall.
        // Clear it so that if THIS turn wedges, the nudge path takes the
        // pure-idle branch and logs a fresh "idle Ns" reason instead of
        // re-reporting the previous turn's failure code.
        s.lastFailure = null
        break
      case 'turn/end': {
        s.turnRunning = false
        const reason = ev.data?.reason
        // User goal: unattended operation. ANY non-completed turn
        // termination should be treated as a "task is stalled, nudge
        // the agent to keep going" event. The only normal exit we
        // ignore is `completed` — that means the agent finished
        // its current task successfully.
        if (reason?.kind === 'completed') {
          ...
          // aborted / interrupted / blocked / max-tokens / unknown —
          // the agent stopped without finishing. Set a synthetic
          // failure so the idle-check path treats this as a
          // failure-fresh session and nudges immediately.
          s.lastFailure = {
            code: 'TURN_' + (reason?.kind ?? 'unknown').toUpperCase(),
            message: 'turn ended with reason.kind=' + (reason?.kind ?? 'unknown'),
          }
        }
        if (typeof ev.data?.turn === 'number') s.lastTurn = ev.data.turn
        break
    ...
    // If a turn ended with anything other than `completed`, run the
    // idle check immediately so the failure-nudge path fires within
    // the next tick. Default 30s is too slow when the user is staring
    // at a stalled conversation; unattended operation requires
    // immediate response on every non-completion.
