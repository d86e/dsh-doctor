 ...
 */
async function detectDshWebLabel(log: DoctorLog): Promise<string | null> {
  if (process.platform === 'darwin') {
    try {
      ...
      await log.warn(`launchctl list failed: ${(e as Error).message}`)
    }
    return null
  }
  if (process.platform === 'linux') {
    try {
      ...
      for (const line of stdout.split('\n')) {
        ...
        if (m && !m[1].includes('doctor')) return m[1]
      }
    } catch (e) {
      await log.warn(`systemctl list failed: ${(e as Error).message}`)
    }
    return null
  }
  if (process.platform === 'win32') {
    try {
      ...
      await log.warn(`schtasks query failed: ${(e as Error).message}`)
    }
    return null
  }
  return null
...
async function main(): Promise<void> {
  ...
  if (webLabel) {
    try {
      ...
      await log.warn(`could not record dsh web label: ${(e as Error).message}`)
    }
  } else {
  ...
  try {
    ...
    await log.warn(`watchdog script write failed: ${(e as Error).message}`)
    process.exit(1)
  }
  ...
  try {
    ...
    await log.warn(`service spec write failed: ${(e as Error).message}`)
    process.exit(1)
  }
  ...
  for (const cmd of [spec.registerCmd, spec.startCmd]) {
    try {
      ...
      await log.warn(`command failed (continuing): ${cmd.join(' ')}: ${(e as Error).message}`)
    }
  }
  ...
  await writeFileAtomic(
    ...
    JSON.stringify(
      { installedAt: new Date().toISOString(), version: 'auto', platform: currentPlatform() },
      null,
      2,
    ),
  )
