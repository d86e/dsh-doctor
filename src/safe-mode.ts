...
export function buildSafeModePatch(allowList: readonly string[]): string {
...
export async function applySafeModePatch(allowList: readonly string[]): Promise<string> {
...
export async function readSafeModePatch(): Promise<string | null> {
  return readFileOrNull(StatePaths.safeModePatch())
}

/** Remove the safe-mode patch. Idempotent. */
export async function clearSafeModePatch(): Promise<void> {
  try {
...
export function safeModePatchPath(): string {
