 ...
 */
function preGt(a: readonly string[], b: readonly string[]): boolean {
  if (a.length === 0 && b.length === 0) return false
  if (a.length === 0) return true // release > prerelease
...
function parsedGtOrEqual(a: ParsedSemver, b: ParsedSemver): boolean {
...
export function satisfiesCaret(version: string, range: string): boolean {
  ...
  // First check >= min.
  if (!parsedGtOrEqual(v, min)) return false

  // Then check upper bound, depending on the leftmost non-zero.
  if (min.maj > 0) return v.maj === min.maj
  if (min.min > 0) return v.maj === min.maj && v.min === min.min
  // 0.0.x — exact core
